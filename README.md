...
- AI 滤镜推荐：GPT 分析当前画面时会返回 `动作 + 滤镜 + 原因`，滤镜返回后实时预览和拍照保存都会使用该滤镜。
...
6. GPT 返回后，取景页会先显示动作建议；如果识别出合适滤镜，App 会自动切换到 AI 推荐滤镜，并显示 `已套用` 和原因。

当前实现会把压缩后的当前取景帧直接发送到 OpenAI API，用于生成取景动作建议和滤镜推荐。生产版本应改成后端代理，避免在客户端保存或暴露 API Key。

## Hermes 相机大脑

服务器上已部署 `ai-camera-agent` Hermes profile，并通过 OpenAI-compatible 接口暴露：
