...
- 手动 Hermes 分析和稳定 5 秒后的自动 Hermes 分析。
...
1. 默认使用 Hermes 相机大脑，模式为 `手动 + 自动`。
2. 对准画面并稳定约 5 秒，App 会自动把当前帧发送给 Hermes 识别。
3. 也可以直接点击取景页的 `识别` 手动触发当前帧分析。
4. Hermes 返回后，取景器会显示主体、移动方向、建议倍率和推荐滤镜；如果返回变焦倍率，App 会自动切换镜头倍率。
5. 如果想换一个主体拍摄，点击 `退出` 清空当前 Hermes 会话，App 会恢复识别前的倍率和滤镜。
6. 只有切到私有网关时才需要填写 API Key；默认公开相机大脑网关由服务器侧鉴权。

当前实现会把压缩后的当前取景帧发送到 Hermes 相机大脑，用于生成取景动作建议、变焦倍率和滤镜推荐。私有网关 API Key 只保存在本机 Keychain，不写入源码。
...
在 App 设置页点击 `使用 Hermes 相机大脑` 会自动填入 Base URL 和模型名。默认公开网关不需要在 App 内填写 API Key；如果切换到私有网关，API Key 会保存到本机 Keychain，不写入源码。
